...
export const TinyToesInvoice: React.FC<TemplateProps> = ({ company, client, document }) => {
    ...
    <div className="bg-blue-50 text-slate-800 p-12 min-h-[800px] w-full max-w-4xl mx-auto font-sans relative shadow-2xl rounded-[3rem] overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-8 mb-12 relative z-10">
         <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-3">Billed To</h3>
         ...
         <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 text-right">
            <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-3">Invoice Date</h3>
            ...
            <div className={`mt-3 inline-block px-3 py-1 text-xs uppercase tracking-widest font-black rounded-lg ${balance <= 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-pink-100 text-pink-500'}`}>
               {balance <= 0 ? 'Fully Paid' : 'Payment Due'}
            </div>
         </div>
      </div>
      ...
      <div className="flex justify-between mb-16 relative z-10">
         <div className="w-64 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
             <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-4">How to Pay</h3>
             <div className="space-y-3 text-sm font-medium text-slate-600">
                {company?.bankDetails?.bankName && <p><span className="text-slate-400 block text-[10px] uppercase">Bank</span> {company.bankDetails.bankName}</p>}
                {company?.bankDetails?.accountNumber && <p><span className="text-slate-400 block text-[10px] uppercase">Account</span> {company.bankDetails.accountNumber}</p>}
                {company?.bankDetails?.ifsc && <p><span className="text-slate-400 block text-[10px] uppercase">IFSC</span> {company.bankDetails.ifsc}</p>}
                {company?.upiId && <p className="mt-4 pt-4 border-t border-slate-100"><span className="text-slate-400 block text-[10px] uppercase">UPI ID</span> {company.upiId}</p>}
             </div>
         </div>
         ...
         <div className="w-72 space-y-4 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div className="flex justify-between items-center text-xl font-black text-slate-800 pt-4 border-t border-slate-100">
               <span>Total</span>
            ...
            <div className={`flex justify-between items-center text-2xl font-black pt-4 border-t border-slate-100 ${balance > 0 ? 'text-pink-500' : 'text-slate-300'}`}>
               <span>Balance</span>
      ...
      <div className="mt-auto relative z-10 bg-white/50 backdrop-blur-md p-6 rounded-3xl text-center border border-white">
         <p className="text-sm font-bold text-slate-500">{company?.invoiceNotes || 'Thank you for your payment!'}</p>
      </div>
    </div>
  );
};
