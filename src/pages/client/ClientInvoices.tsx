  ...
  const handleViewPdf = async (invoice: Invoice) => {
    ...
    try {
      ...
      if (newTab) {
        ...
        newTab.document.title = `${invoice.type === 'quotation' ? 'Quotation' : 'Invoice'} ${invoice.id}`;
      } else {
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 w-full md:w-auto">
             ...
             <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                   ...
                   {isQuote ? (
                      <span className={`px-2.5 py-1 text-[8.5px] font-black uppercase rounded-lg tracking-widest ${invoice.status === 'Approved' || invoice.status === 'Paid' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'}`}>
                         {invoice.status === 'Approved' || invoice.status === 'Paid' ? 'Approved' : 'Review Required'}
                      </span>
                   ) : (
                   ...
                   <button
                      onClick={() => {
                         if (invoice.status !== 'Approved' && invoice.status !== 'Paid') {
                            window.location.href = `/agreement/${invoice.id}`;
                         }
                      }}
                      className={`w-full sm:w-auto px-6 py-4 ${invoice.status === 'Approved' || invoice.status === 'Paid' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-primary text-black hover:opacity-90'} text-xs font-bold uppercase tracking-widest rounded-xl transition-colors active:scale-95`}
                      disabled={invoice.status === 'Approved' || invoice.status === 'Paid'}
                   >
                      {invoice.status === 'Approved' || invoice.status === 'Paid' ? 'Approved' : 'Accept Quotation'}
                   </button>
                ) : (
                   <button
