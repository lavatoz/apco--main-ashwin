...
import { type Client, type Task, type Booking, TaskStatus, type Company } from '../types';
...
const ProductionHub: React.FC<ProductionHubProps> = ({ companies, clients, tasks, bookings, selectedBrand, onOpenClient }) => {
  const [activeTab, setActiveTab] = useState<'all' | 'ops' | 'delivery'>('all');
  const [projects, setProjects] = useState<any[]>([]);
  console.log(projects);

  const activeCompany = companies.find((c: any) => c.name === selectedBrand);
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const url = selectedBrand === 'All' || !selectedBrandId
            ? 'http://localhost:5000/api/clients'
            : `http://localhost:5000/api/clients?brandId=${selectedBrandId}`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-8 squircle-lg space-y-8 bg-zinc-900/20">
          <div className="flex items-center justify-between border-b border-white/5 pb-6">
            ...
            <span className="text-[9px] font-black uppercase text-zinc-500">{projects.length} In Progress</span>
          </div>
            ...
            {projects.length === 0 ? (
              ...
              <div key={client.id} onClick={() => onOpenClient(client)} className="p-5 bg-white/5 rounded-[1.5rem] hover:bg-white/10 transition-all cursor-pointer group border border-white/5 relative overflow-hidden">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="font-black text-xs uppercase tracking-tight text-white">{client.name || client.projectName}</h4>
                    <p className="text-[9px] font-black uppercase text-zinc-600 tracking-widest mt-1">{client.brand || 'No Brand'} • {client.eventDate ? new Date(client.eventDate).toLocaleDateString() : 'TBD'}</p>
                  </div>
